...
#This exmaple is demonstrated using probablity of infant vocalistion receiving adult response. Change files and variables as necessary
...
ch_fst = read.csv('chvoc_logisticreg_stepsizes_HUM.csv', header = TRUE)
...
chn_fstep_lmer_model = glmer(response ~ (1|id) + (1|listener) + scale(freq_dist) + scale(infantage) + scale(amp_dist) + scale(time_dist), data = ch_fst, family=binomial())
