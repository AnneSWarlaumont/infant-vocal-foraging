function [ch_mean_steps,T_binary,T_2voc] = meanstsizes_fn_LENA(adr_st,adr_en,adresponse,adr_zlogf,adr_zd,id1,age1)

%Ritwika VPS, UC Merced

%functionalised version of code that computed mean step sizes at nth
%vocalisations since last response

%we'll find distances in f space, d space, vocal space, and time, as well
%as mean velocities
...
dis_f = dis_f(r_ind:end); %cut down distance and velocuty matrices to the sizes compatible with updated response matrix
...
for i = 1:length(id1)  
    id_age{i} = sprintf('%s_%d',id1{i},age1(i));
end
...
for i  = 1:length(id_age)
    ...
    if (strcmp(id_age{i},sprintf('%s_%d',id1{j},age1(j))) == 1) && (isempty(distf_subrec{j}) == 0) 
...
[mean_v_sp,std_v_sp,CI_v_sp_neg,CI_v_sp_pos,num_v_sp] = mean_std_95CI(velocity_sp_mat);

[mean_d_sp,std_d_sp,CI_d_sp_neg,CI_d_sp_pos,num_d_sp] = mean_std_95CI(distance_sp_mat);
[mean_d_f,std_d_f,CI_d_f_neg,CI_d_f_pos,num_d_f] = mean_std_95CI(distance_f_mat);
[mean_d_d,std_d_d,CI_d_d_neg,CI_d_d_pos,num_d_d] = mean_std_95CI(distance_d_mat);
[mean_d_t,std_d_t,CI_d_t_neg,CI_d_t_pos,num_d_t] = mean_std_95CI(distance_t_mat);

ch_mean_steps = table(mean_v_sp,std_v_sp,CI_v_sp_neg,CI_v_sp_pos,num_v_sp,...
