...
%for adult vocalisations, human labelled data
...
for i  = 1:length(id_age)
    ...
    if (strcmp(id_age{i},sprintf('%s_%d_%s',childid_ad{j},age_adhumlab(j),listenerid_ad{j})) == 1) && (isempty(dis_sp_subrec{j}) == 0)  
        
        distsp_day{i} = [distsp_day{i} dis_sp_subrec{j}];
        disttim_day{i} = [disttim_day{i} dis_t_subrec{j}];
        f_day{i} = [f_day{i} transpose(adlogf_humlab{j})];
        d_day{i} = [d_day{i} transpose(addb_humlab{j})];
...
for i = 1:length(id_age)
    mean_f_ad(i,1) = nanmean(f_day{i});
    std_f_ad(i,1) = nanstd(f_day{i});
    max_f_ad(i,1) = max(f_day{i});
    min_f_ad(i,1) = min(f_day{i});
    
    mean_d_ad(i,1) = nanmean(d_day{i});
    std_d_ad(i,1) = nanstd(d_day{i});
    max_d_ad(i,1) = max(d_day{i});
    min_d_ad(i,1) = min(d_day{i});
