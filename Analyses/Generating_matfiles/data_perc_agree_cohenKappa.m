...
%listeners. However, human listeners listened to sounds which had labels CHN, FAN, MAN,
%CHNNSP and CXN according to LENA. In addition, human listeners sometimes
%(presumably) skipped some portion of the recordings (this part needs
%clarification from Tim). So, in this code, we go back to the larger
%segments files to identify all CXN, CHNNSP, CHNSP, FAN and MAN labels from
...
%Also note that (presumably) both CHNSP and CHNNSP labels from LENA are
...
%relying on LENA labels, this will be our approach: All CHN labels by human listeners devoid
...
%pick out all files that are segemnts for infant 530 at 95 days
...
for i = 1:length(hum_data)
    ...
    for k = 1:length(spsp)
        if regexp(spsp{k},chexp) == 1
            newspsp{k} = 'CHNSP';
        elseif regexp(spsp{k},adexpf) == 1
            newspsp{k} = 'FAN';
        elseif regexp(spsp{k},adexpm) == 1
            newspsp{k} = 'MAN';  
        else
            newspsp{k} = 'NA';
        end
    ...
    for hh = 1:length(humdata_childidmatch)-1 %exclude 530 by L3
        ...
        if strcmp(h_child,l_child) == 1 %compare if child ids from lena and human labelled data are the same
            [cc,i_l,i_h] = intersect(l_table.startsec,h_table.start);
             ...
             T = table(new_start,new_lena_speaker,new_hum_speaker);
             
             new_lena_name = sprintf('%s_%s_interraterreliability.csv',l_child,h_listener);
             writetable(T,new_lena_name)
