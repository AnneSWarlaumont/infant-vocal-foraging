...
for i  = 1:length(id_age)
    ...
    if (strcmp(id_age{i},sprintf('%s_%d',adr_id{j},adr_age(j))) == 1) && (isempty(dis_sp_subrec{j}) == 0)  
        
        distsp_day{i} = [distsp_day{i} dis_sp_subrec{j}];
        disttim_day{i} = [disttim_day{i} dis_t_subrec{j}];
        f_day{i} = [f_day{i} transpose(adr_zlogf{j})];
        d_day{i} = [d_day{i} transpose(adr_zd{j})];
...
for i = 1:length(id_age)
    if isempty(f_day{i}) == 0 %make sure that the vector is not empty
    i_count = i_count+1;
    mean_f_ch(i_count,1) = nanmean(f_day{i});
    std_f_ch(i_count,1) = nanstd(f_day{i});
    max_f_ch(i_count,1) = max(f_day{i});
    min_f_ch(i_count,1) = min(f_day{i});
    
    mean_d_ch(i_count,1) = nanmean(d_day{i});
    std_d_ch(i_count,1) = nanstd(d_day{i});
    max_d_ch(i_count,1) = max(d_day{i});
    min_d_ch(i_count,1) = min(d_day{i});
