...
%following metrics: turning angle in space, velocity in space + other
%dimensions + step sizes, all for only the immediate first vocalisation
...
writetable(T_aic,'chresp2ad_1v_aicdiff.csv')
...
writetable(T_disc,'chr2ad_stepsizedist_arearest.csv')
save('chresp2ad_arearest_stepsizes.mat','distf_ch_day','distd_ch_day','distsp_ch_day','disttim_ch_day','velsp_ch_day'...
    ,'distf_noch_day','distd_noch_day','distsp_noch_day','disttim_noch_day','velsp_noch_day','id_age')
writetable(numberoffits_disc,'fittypes_chr2ad_arearest.csv')
