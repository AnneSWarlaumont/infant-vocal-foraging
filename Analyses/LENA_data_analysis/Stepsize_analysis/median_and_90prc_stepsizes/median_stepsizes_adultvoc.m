...
%for adult vocalisations (CHN responses)
...
%we'll find distances in f, d, vocal space, and time
...
dis_f = dis_f(r_ind:end); %cut down distance matrices to the size of updated response matrix
dis_d = dis_d(r_ind:end);
dis_sp = dis_sp(r_ind:end);
dis_t = dis_t(r_ind:end);

%remove nan values - space steps would have any nan values for bth f and
...
%The easiest way to find the median/90prc across all
...
%These can tehen be stacked together into a single matrix to find median/90prc
%So, a cell array corresponding to one data set will have matrices of the
...
%make sure to change the name of the output file accordingly
...
writetable(chn_prctile_steps,'chresp2ad_median_stepsize_alldatatogether.csv')
