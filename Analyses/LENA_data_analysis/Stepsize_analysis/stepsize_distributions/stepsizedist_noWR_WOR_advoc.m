...
for i  = 1:length(id_age)
    ...
    if (strcmp(id_age{i},sprintf('%s_%d',chr2ad_id{j},chr2ad_age(j))) == 1) && (isempty(distsp_subrec{j}) == 0) 
...
rsq_tab = table(age,id,smplsi,rsq_f_advoc,rsq_d_advoc,rsq_sp_advoc,rsq_t_advoc); %writes rsq table
writetable(rsq_tab,'rsq_advoc_noWR_WOR.csv') %writes table to file

save('advoc_stepsizes_noWR_WOR.mat','distf_day','distd_day','distsp_day','disttim_day','id_age')
...
%recording the kind of fits for each category%recording the kind of fits for each category
