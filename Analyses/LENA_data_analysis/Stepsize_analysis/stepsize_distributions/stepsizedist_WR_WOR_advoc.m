...
smplsi(jj,1) = length(distf_ch_day{j})+length(distf_noch_day{j});
smplsi_WR(jj,1) = length(distf_ch_day{j});
smplsi_WOR(jj,1) = length(distf_noch_day{j});
...
rsq_tab = table(age,id,smplsi_WR,smplsi_WOR,rsq_f_ch,rsq_d_ch,rsq_sp_ch,rsq_t_ch,rsq_f_noch,rsq_d_noch,rsq_sp_noch,rsq_t_noch); %writes rsq table
...
T_disc = table(id,age,expf,expd,lognspmu,lognspsig,paretotimxmin,paretotimsig,smplsize,response,f_fit,d_fit,sp_fit,tim_fit);
