...
#linear mixed effects model to test the effect of age, and optional sample size on correlation between step sizes in time and acoustic space. Thsi script can also used for linear mixed effects stats for other measures that do not have response received as an indepednent variable (eg. mean, max and min of individual acoustic dimensions)
#This exmaple is demonstrated using correlation of step sizes in acoustic space and time for adult vocalisations. Change files and variables as necessary
library(lme4)
library(lmerTest)
...
ch_fst = read.csv('advoc_corrltn_mean.csv', header = TRUE)
...
chn_fstep_lmer_model = lmer(scale(corrltn_ad) ~ (1|id) + scale(age) + scale(samplsi), data = ch_fst)
